...
@Component({
  ...
  templateUrl: './medico-permiso.component.html',
  styleUrl: './medico-permiso.component.css',
  providers: [
    {
...
export class MedicoPermisoComponent implements OnInit, AfterViewInit {
    ...
    this._rolPermisoServicio.lista().subscribe({
      next: (data) => {
        if (data.estado) {
          ...
          this.dataListaPermisosRoles.data = this.dataListaPermisosRoles.data.filter(rp => rp.rolNombre === 'Medico');
        } else {
  ...
  nuevoRolPermiso() {
    this.dialog.open(ModalPermisoMedicoComponent, {
      disableClose: true
